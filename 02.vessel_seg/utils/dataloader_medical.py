...
class UnetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        jpg = Image.open(os.path.join(self.dataset_path, "Images", name + ".png"))
        ...
        # Label modification (use 1 for values <= 127.5)
        modify_png = np.zeros_like(png)
        modify_png[png <= 127.5] = 1
        seg_labels = np.eye(self.num_classes + 1)[modify_png.reshape([-1])]
    ...
    def get_random_data(self, image, label, input_shape, jitter=0.3, hue=0.1, sat=0.7, val=0.3, random=True):
        ...
        # Apply random scaling and aspect ratio distortion
        ...
        # Add gray bars and apply random placement
